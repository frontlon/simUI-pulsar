...
func inputIniConfig(p string) error {
	...
	//写入rom数据
	starList := []string{}
	_ = json.Unmarshal([]byte(utils.Base64Decode(file.Section("rom").Key("star").String())), &starList)
	hideList := []string{}
	_ = json.Unmarshal([]byte(utils.Base64Decode(file.Section("rom").Key("hide").String())), &hideList)
	simList := map[string]string{}
	_ = json.Unmarshal([]byte(utils.Base64Decode(file.Section("rom").Key("sim").String())), &simList)

	simIdList := map[string][]string{}
	_ = json.Unmarshal([]byte(utils.Base64Decode(file.Section("rom").Key("simId").String())), &simIdList)
	//合并name，根据name读取ids
	nameList := []string{}
	...
	_ = (&db.Rom{}).UpdateStarByIds(starIds, 1)
	...
	_ = (&db.Rom{}).UpdateHideByIds(hideIds, 1)
	...
	for k, v := range simIdList {
		...
		_ = (&db.Rom{}).UpdateSimIdByIds(simListIds, simIds[k])
	}

	//更新sim_conf到数据库
	for romName, v := range simList {
		romData := map[string]map[string]string{}
		_ = json.Unmarshal([]byte(v), &romData)
		...
		_ = (&db.Rom{}).UpdateSimConfById(romId, string(create))
	}
	...
	if simSection != nil {
		...
		for _, v := range sims {
			sim := &db.Simulator{
				...
				Default:  uint8(utils.ToInt(v.Key("index").String())),
