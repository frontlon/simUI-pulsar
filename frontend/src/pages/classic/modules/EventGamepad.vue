...
<script lang="ts">
import {ref} from "vue";
import {clickGameEvent, romEventKeyboard, runActiveGame} from 'src/pages/classic/ContentBar.vue'

import {getMilliSeconds, notify} from "components/utils";
import {clickMenuEvent, menuEventKeyboard} from "pages/classic/LeftBarMenu.vue";
import {clickPlatformEvent, platformEventKeyboard} from "pages/classic/LeftBarPlatform.vue";
...
function gamepadControlButton(buttons: ReadonlyArray<GamepadButton>) {
  for (let i = 0; i < buttons.length; i++) {
    if (buttons[i].value == 1) {
      ...
      } else if (i == 4) { //摇杆5，左手LT
        console.log("摇杆5，左手LT");
        platformEventKeyboard("ArrowUp")
      } else if (i == 5) { //摇杆6，右手RT
        menuEventKeyboard("ArrowUp")
        console.log("摇杆6，右手RT");
      } else if (i == 6) { //摇杆投币，左手LB
        console.log("摇杆投币，左手LB");
        platformEventKeyboard("ArrowDown")
      } else if (i == 7) { //摇杆开始，右手RB
        console.log("摇杆开始，右手RB");
        menuEventKeyboard("ArrowDown")
      } else if (i == 9) { //手柄START
        console.log("手柄START");
        runActiveGame()
